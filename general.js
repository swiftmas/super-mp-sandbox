...
function ticBegin() {
  ...
  for (var player in dp) {
    ...
    listener.sockets.connected[player.slice(1)].emit('camera', [dp[player].pos, dp[player].health, dp[player].maxHealth, dp[player].mana, dp[player].maxMana, dp[player].cor, dp[player].maxCor, globals.weaponData[dp[player].inventory[0].name].sprite, globals.weaponData[dp[player].inventory[0].name].sprite, globals.weaponData[dp[player].inventory[1].name].sprite, globals.weaponData[dp[player].inventory[2].name].sprite])
...
function ProcessTime() {
  ...
  if (globals.time == 3000) {
    ...
    listener.sockets.emit('serverMessage', {
      "message": globals.serverMessage,
      "time": globals.time
    })
  }
  if (globals.time == 0) {
    ...
    getDist(location, chunk, function(result) {
      if (Math.abs(result[1]) < (128 + (width / 2)) && Math.abs(result[2]) < (128 + (height / 2))) {
        for (var part in collideableParts) {
          if (collideableParts[part] == "players" && playersHaveBeenRun !== true) {
            var db = coredata[collideableParts[part]];
            playersHaveBeenRun = true;
            chunk = "none"
          } else {
            var db = coredata.chunks[chunk][collideableParts[part]]
          };
          for (var item in db) {
...
function ProcessMovements() {
  ...
  // console.warn("MOVEMENT IS PROCESSING")
};
...
function DoMovement(name, chunk, dir, rate, ignoreCollision, maintainFacingDirection) {
  ...
  if (maintainFacingDirection == null) {
    maintainFacingDirection = true
  };
  var db, nameType
  switch (name[0]) {
    case "n":
  ...
  if (db[nameType][name].state == 0) {
    db[nameType][name].state = 3
  }
...
// console.warn("MOVEMENT IS HAPPENING!");
};
